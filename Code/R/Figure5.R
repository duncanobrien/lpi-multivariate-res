...
ggplot2::ggsave("Results/figures/figure5.pdf",
                ggplot(subset(summary_range_data,metric == "multiJI"),aes(x=.value,y=n_spp,group = stressed)) +
                  ...
                  scale_color_manual(values = c("#67705F","#E3A59F"),
                                     guide = "none")+
                  xlab("Probability of exceeding 1") + ylab("Community size\n(#species)") +
                  scale_x_continuous(labels = round(plogis(labels),1), breaks = breaks)+
                  coord_trans(x = inv_logit_perc,xlim = c(-4.5,4.5))+
                  ggtitle("multiJI") +
                  theme_bw() +
                  ...
                  scale_color_manual(values = c("#67705F","#E3A59F"),
                                     guide = "none")+
                  xlab("Probability of exceeding threshold") + ylab("Community size\n(#species)") +
                  scale_x_continuous(labels = round(plogis(labels),1), breaks = breaks)+
                  coord_trans(x = inv_logit_perc,xlim = c(-4.5,4.5))+
                  ggtitle("multiAR") +
                  theme_bw() +
